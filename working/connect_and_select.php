<?php
// Sample SELECT usign PDO query()

// database username
// $sweetscomplete = 'sweetscomplete';

// database password
$pass = '';

// data source = mysql driver, localhost, database = class
// $dsn = new PDO($dsn, $sweetscomplete, $pass);
$dsn = 'mysql:host=localhost;dbname=sweetscomplete';

// SQL statement
$sql = 'SELECT * 
FROM  `products` 
LIMIT 0 , 30';

// Use query() for "one-time" SQL requests
// PDO::FETCH_ASSOC = return results in the form of an associative array
echo '<h3>Products</h3>', '<hr />', PHP_EOL;
echo '<table border=1>', PHP_EOL;
foreach ($dbh->query($sql, PDO::FETCH_ASSOC) as $row) {
    // each $row = an associative array representing one row in the database
    // the key = the column name
    echo '<tr><td>', implode('</td><td>', $row), '</td></tr>', PHP_EOL;
}
echo '</table>', PHP_EOL;

// closes the database connection
$dbh = NULL;